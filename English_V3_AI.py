...
st.sidebar.write("🔑 Supabase key loaded:", bool(supabase_key))
try:
...
QUIZZES = {
    ...
    "A2 - Elementary": [
        (
            "Yesterday, I ___ my friend.",
            ["visit", "visited", "visiting"],
            "visited"
        ),
        (
            "I think it ___ tomorrow.",
            ["will rain", "rained", "rains"],
            "will rain"
        ),
        (
            "A car is ___ than a bicycle.",
            ["fast", "faster", "fastest"],
            "faster"
        ),
    ],
    ...
    "B2 - Upper-Intermediate": [
        ...
        (
            "___ it was difficult, we continued.",
            ["Although", "Because", "So"],
            "Although"
        ),
    ],
...
st.caption(
    "منصة تفاعلية لتعلم الإنجليزية خطوة بخطوة"
)
...
if choice == "🏠 Home":
    ...
    st.write(
        "تعلم الإنجليزية من خلال الدروس والمفردات "
        "والتمارين والمحادثة."
    )
...
# ============================================================
    ...
    for lesson_id, title, summary in LEVELS[level]["lessons"]:
            ...
            lesson_id
            in st.session_state.completed_lessons
        )
...
# ============================================================
    ...
    for lesson_id, title, summary in LEVELS[level]["lessons"]:
        ...
        with st.expander(
            f"{'✅' if lesson_id in st.session_state.completed_lessons else '📘'} {title}"
        ):
            ...
            if title == "Present Simple":
                ...
                st.write(
                    "نستخدم Past Simple للحديث عن "
                    "أحداث انتهت في الماضي."
                )
                ...
                st.write(
                    "نستخدم will للحديث عن المستقبل "
                    "والتوقعات والقرارات."
                )
            ...
            if st.button(
                ...
                key=f"done_{lesson_id}"
            ):

                mark_lesson_done(lesson_id)

                st.success(
                    "🎉 تم إكمال الدرس وحصلت على 10 نقاط!"
                )
...
# ============================================================
    ...
    st.info(
        "💡 حاول كتابة جملة جديدة باستخدام كل كلمة."
    )
...
# ============================================================
    ...
    if st.button(
        ...
        previous = st.session_state.quiz_scores.get(
            level,
            0
        )

        st.session_state.quiz_scores[level] = max(
            previous,
            score
        )
...
# ============================================================
    ...
    if not os.getenv("OPENAI_API_KEY"):
        ...
        st.info(
            "أضف مفتاح OpenAI في متغير البيئة "
            "OPENAI_API_KEY قبل تشغيل المحادثة."
        )
...
# ============================================================
    ...
    if st.session_state.quiz_scores:
        ...
        for level, score in (
            st.session_state.quiz_scores.items()
        ):

            st.write(
                f"**{level}:** "
                f"{score}/{len(QUIZZES[level])}"
            )
